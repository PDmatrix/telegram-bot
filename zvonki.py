...
#Возвращает номер текущей пары если пар нет то 0, если перемена то -1
def get_para_num():
	para_num = 0
	for idx, para in enumerate(raspisanie()):
		...
		try:
			para_next = datetime.strptime(raspisanie()[idx+1][0], "%H:%M")
		except:
			pass
		#Проверка на пару
		...
		if current_time()  > para_end and current_time() < para_next:
			para_num = -1
	return para_num
